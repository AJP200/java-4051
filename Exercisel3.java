...
public class Exercisel3 {
    ...
    public static void main(String[] args) {
            ...
            double [] test_storage = Exercisel3Methods.readTestScores(n);
            Exercisel3Methods.printTestResults(test_storage);

        }
}

class Exercisel3Methods{
    static double [] readTestScores(int size){
        double [] test_scores = new double[size];

        for(int i = 0; i < size; i++){
            System.out.printf("Test score %d: ", i+1);
            test_scores[i] = Exercisel3.scan.nextDouble();
        }
         return (test_scores);
    }
